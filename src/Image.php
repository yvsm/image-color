...
namespace zunyunkeji\imageColor;

use Exception;
use zunyunkeji\imageColor\Object;
...
class Image extends Object
{
    ...
    protected $_classes = array(
        'color' => 'zunyunkeji\imageColor\Color',
    );
    ...
    /**
     * @throws Exception If the image path cannot be found or
    ...
    public function load($file)
    {
        if (is_string($file)) {
            if (!file_exists($file)) {
                throw new Exception("Image not found: {$file}");
            }
            $info = pathinfo($file);
            if (!isset($this->_fileTypes[$info['extension']])) {
                throw new Exception("Image not supported: {$info['extension']}");
            }
        ...
        throw new Exception('Resource or image path not given.');
    }
